...
import { Lock, UserCircle, ArrowLeft, RefreshCw, Mail } from 'lucide-react';
...
export function Signup() {
  ...
  const { loginAsGuest, signInWithOtp } = useAuth();
  
  const validateEmail = (e: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e);
  const isValid = validateEmail(email) && fullName.length > 0;
    ...
    <div className="min-h-screen bg-[#0a0a0c] text-[#f5f5f7] flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-[360px] relative z-10">
        <div className="flex flex-col items-center mb-6 text-center">
          <Logo className="mb-4" />
          <h1 className="text-[24px] font-semibold tracking-tight text-white mb-2 leading-tight">Create your account</h1>
          <p className="text-[13px] text-[#86868b]">Set up your cryptographic audit trail.</p>
        </div>
        ...
        {view === 'social' ? (
          <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <button 
              ...
              className="w-full h-[50px] bg-transparent border border-white/10 hover:bg-white/5 text-white rounded-[40px] flex items-center justify-center gap-3 transition-transform hover:scale-[1.02] active:scale-[0.98] font-medium text-[15px]"
            >
            ...
            <p className="mt-5 text-center text-[13px] text-[#f5f5f7] font-medium">
              ...
              <Link to="/auth/login" className="text-white hover:underline underline-offset-4 font-bold">
                Log in
              </Link>
            </p>
            ...
            <form onSubmit={handleSignup}>
              <div className="mb-4">
                <input 
                  ...
                  className={`w-full h-[50px] bg-[#2c2c2e] border-transparent focus:border-white/20 border rounded-[14px] px-4 text-[14px] text-white placeholder:text-[#86868b] transition-all outline-none focus:ring-4 focus:ring-white/5`}
                />
              </div>
              <div className="mb-5">
                <input 
                  ...
                  className={`w-full h-[50px] bg-[#2c2c2e] ${error ? 'border-red-500/50 focus:border-red-500' : 'border-transparent focus:border-white/20'} border rounded-[14px] px-4 text-[14px] text-white placeholder:text-[#86868b] transition-all outline-none focus:ring-4 focus:ring-white/5`}
                />
