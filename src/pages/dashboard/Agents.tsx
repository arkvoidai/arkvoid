...
// Agent limits per plan — must match PLAN_LIMITS in constants.ts
...
export function Agents() {
  ...
  const agentLimit = AGENT_LIMITS[planKey] ?? 3;
  ...
  const filteredAgents =
    agents?.filter((agent: any) => {
      if (
        ...
        !(agent.slug || '').toLowerCase().includes(debouncedSearch.toLowerCase()) &&
        !(agent.agent_type || agent.type || 'custom').toLowerCase().includes(debouncedSearch.toLowerCase())
      ) {
        return false;
      }
  ...
  const handleRegisterClick = () => {
    ...
    if (agentLimit !== Infinity && (agents?.length || 0) >= agentLimit) {
  ...
  return (
    // FIX: Removed centering classes that caused "ARKVOID INC." to appear mid-screen.
    // The layout must be flex-col with full width, NOT items-center.
        ...
        <div className="flex items-center gap-4 w-full min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 group relative">
              <h1 className="text-[20px] font-semibold text-[var(--text-primary)] cursor-pointer">
                Agent Registry
              </h1>
              <HelpCircle className="w-4 h-4 text-[var(--text-tertiary)] cursor-pointer shrink-0" />
              <div className="absolute top-full left-0 mt-2 w-[280px] p-3 bg-[#111] text-[12px] leading-relaxed text-[var(--text-secondary)] border border-[var(--border-subtle)] rounded shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 pointer-events-none">
                An Agent is any AI system you want to monitor — a chatbot, an automated workflow, a document
                processor, or any code that uses AI. Each agent gets a unique identity and a complete record of
                everything it does.
              </div>
            </div>
      ...
      {hasNoAgentsAtAll ? (
          ...
          <div className="w-full max-w-[900px] text-left">
            <details className="mb-12 group">
              ...
              <div className="mt-4 p-5 bg-[#0a0a0a] border border-[var(--border-subtle)] rounded-lg text-[13px] text-[var(--text-secondary)] leading-relaxed">
                An agent is any AI system you want to monitor.
                <br />
                <br />
                Examples: a ChatGPT-powered chatbot, a document processor, a financial analysis tool, or any code
                that calls an AI API.
                <br />
                Give it a name and ARKVOID creates a unique identity and audit trail for it.
              </div>
            </details>

            <h3 className="text-[13px] font-medium text-[var(--text-tertiary)] uppercase tracking-wider mb-4">
              Examples of configured agents
            </h3>

            <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 opacity-40 pointer-events-none select-none">
              {[
                { name: 'Customer Support Bot', status: 'Active', color: 'bg-green-500', model: 'gpt-4-turbo' },
                { name: 'Document Processor', status: 'Idle', color: 'bg-blue-500', model: 'claude-3-opus' },
                { name: 'Financial Analyst', status: 'Training', color: 'bg-orange-500', model: 'gemini-1.5-pro' },
              ].map((ex) => (
                <div
                  key={ex.name}
                  className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl p-5 relative grayscale"
                >
                  <div className="absolute top-3 right-3 text-[10px] uppercase tracking-wider font-bold bg-white/10 text-white px-2 py-1 rounded">
                    Example
                  </div>
                  <div className="flex items-start justify-between mb-4 mt-2">
                    <div>
                      <h3 className="text-[15px] font-semibold text-white truncate pr-4">{ex.name}</h3>
                      <div className="text-[12px] text-[var(--text-tertiary)] flex items-center gap-1 mt-1">
                        <span className={`w-1.5 h-1.5 rounded-full ${ex.color}`} /> {ex.status}
                      </div>
                    </div>
                  </div>
                  <div className="text-[12px] text-[var(--text-secondary)]">{ex.model}</div>
                  <div className="mt-4 pt-4 border-t border-[var(--border-subtle)] flex items-center justify-between text-[11px] text-[var(--text-tertiary)]">
                    ...
                    <span className="font-mono bg-[var(--bg-primary)] px-2 py-0.5 rounded border border-[var(--border-default)] font-medium">
                      ag_...
                    </span>
                  </div>
                </div>
        ...
        <>
          ...
          <div className="sticky top-0 z-10 bg-[var(--bg-primary)] border-b border-[var(--border-subtle)] px-4 sm:px-6 py-3 flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-6">
            ...
            <div className="ml-auto text-[13px] text-[var(--text-tertiary)] shrink-0">
              ...
              <div className="grid grid-cols-[repeat(auto-fill,minmax(min(100%,320px),1fr))] gap-4">
                {filteredAgents.map((agent: any, index: number) => (
                  <div
                    ...
                    style={{ animationDelay: `${index < 10 ? index * 30 : 0}ms` }}
                  >
