...
import { Grid, Cpu, Activity, Shield, CheckCircle, Key, Settings, Puzzle, ChevronLeft, ChevronRight, Menu, Bell, HelpCircle, Plus, ShieldCheck, MailOpen } from 'lucide-react';
...
// ─── Plan trace limits — keep in sync with constants.ts ─────────────────────
const PLAN_TRACE_LIMITS: Record<string, number> = {
  FREE: 25_000,
  PRO: 500_000,
  TEAM: 5_000_000,
  ENTERPRISE: Infinity,
...
const NavItem: React.FC<{
  ...
  badgeCount?: number;
  onMobileClick: () => void;
}> = ({ item, collapsed, isActive, badgeCount, onMobileClick }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center">
        {isActive && <div className="absolute left-[-8px] w-[2px] h-full bg-[var(--accent-amber)]" />}
        <Icon
          className={`w-4 h-4 shrink-0 ${
            isActive ? 'text-[var(--accent-amber)]' : 'text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]'
          }`}
        />
...
export function DashboardLayout() {
  ...
  const planKey: string = user?.user_metadata?.plan || 'FREE';
  const traceLimit = PLAN_TRACE_LIMITS[planKey] ?? 25_000;
  const isPaidPlan = planKey !== 'FREE';
  const monthTraces = dashboardData?.monthTraces || 0;
  const tracePct = traceLimit === Infinity ? 0 : Math.min((monthTraces / traceLimit) * 100, 100);
  const traceLimitDisplay = traceLimit === Infinity ? '∞' : traceLimit >= 1_000_000
    ? `${traceLimit / 1_000_000}M`
    : traceLimit >= 1_000
    ? `${traceLimit / 1_000}K`
    : String(traceLimit);
  const planLabel = planKey === 'FREE' ? 'Free' : planKey === 'PRO' ? 'Pro' : planKey === 'TEAM' ? 'Team' : planKey;
  ...
  useEffect(() => {
    if (user && !isGuest) {
      const fetchAllData = async () => {
        ...
        const newLayoutData = { pendingReviews: cachedLayout.pendingReviews, unreadAnomalies: cachedLayout.unreadAnomalies, notifications: cachedLayout.notifications };

        if (count != null) { setPendingReviews(count); newLayoutData.pendingReviews = count; }
        if (anomaliesData) { setUnreadAnomalies(anomaliesData); newLayoutData.unreadAnomalies = anomaliesData; }
        if (notifsData) { setNotifications(notifsData); newLayoutData.notifications = notifsData; }
      ...
      const sub = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications', filter: `user_id=eq.${user.id}` }, (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setLiveToast({ id: toastId, title: newNotif.title, message: newNotif.message, type: newNotif.type, link: newNotif.link });
            setTimeout(() => setLiveToast((prev) => (prev?.id === toastId ? null : prev)), 5000);
          }
        })
        .subscribe();
  ...
  useEffect(() => {
    if (user && !isGuest && dashboardData) {
      if (dashboardData.monthTraces > 20000) trackPLGSignal(user.id, 'limit_approaching');
      if (dashboardData.allAgents?.length >= 5) trackPLGSignal(user.id, 'multiple_agents_5plus');
      if (dashboardData.weekTraces > 5000) trackPLGSignal(user.id, 'power_user');
    }
  }, [user, isGuest, dashboardData]);

  // Nudge after 7 days on free plan
  useEffect(() => {
    if (user && !isPaidPlan && !localStorage.getItem('arkvoid_nudge1_dismissed')) {
      const created = new Date(user.created_at || Date.now());
      const diffDays = Math.floor((Date.now() - created.getTime()) / (1000 * 3600 * 24));
      if (diffDays >= 7) localStorage.setItem('arkvoid_show_nudge1', 'true');
    }
  }, [user, isPaidPlan]);
  ...
  const getNotifColor = (type: string) => {
    switch (type) {
      ...
      case 'compliance_drop': case 'policy_triggered': return 'bg-red-500';
      default: return 'bg-gray-500';
    }
  };

  // ── Usage bar (sidebar) ───────────────────────────────────────────────────
  const UsageBar = ({ small = false }: { small?: boolean }) => {
    if (isPaidPlan) return null;
    const textSize = small ? 'text-[10px]' : 'text-[11px]';
    const nearLimit = monthTraces > traceLimit * 0.95;
    const approaching = monthTraces > traceLimit * 0.80;
    return (
      <div className={`${small ? 'px-4 py-3' : 'px-5 py-4'} border-t border-[var(--border-subtle)]`}>
        <div className="flex justify-between items-center mb-1.5">
          <span className={`${textSize} font-bold text-[var(--text-secondary)] uppercase tracking-wider`}>
            Usage ({planLabel})
          </span>
          <span className={`${textSize} text-[var(--text-secondary)]`}>
            {monthTraces.toLocaleString()} / {traceLimitDisplay}
          </span>
        </div>
        <div className="w-full h-1.5 bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-full overflow-hidden">
          <div
            className={`h-full transition-all ${
              nearLimit ? 'bg-[var(--status-danger)]' : approaching ? 'bg-[var(--accent-amber)]' : 'bg-[var(--accent-amber)]/80'
            }`}
            style={{ width: `${tracePct}%` }}
          />
        </div>
        {nearLimit && (
          <div className={`mt-2 ${textSize} text-[var(--status-danger)] font-medium text-center`}>
            ⚠️ Almost at limit.{' '}
            <Link to="/dashboard/settings?tab=billing" className="underline">
              Upgrade
            </Link>
          </div>
        )}
    ...
    <div className="flex h-screen bg-[var(--bg-primary)] overflow-hidden text-[var(--text-primary)] font-sans">
      ...
      >
        ...
        <div className="h-[52px] flex items-center px-2 border-b border-[var(--border-subtle)] relative">
          <div className="flex items-center flex-1 min-w-0" style={{ marginLeft: collapsed ? '0' : '4px' }}>
        ...
        <div className="flex-1 overflow-y-auto py-2 hide-scrollbar">
          {!collapsed && (
            <div className="text-[10px] text-[var(--text-tertiary)] tracking-[0.08em] uppercase mt-2 mb-1 px-3">
              Workspace
            </div>
          )}
          <div className="mb-4">
            {workspaceItems.map((item) => (
              <NavItem
                ...
                badgeCount={item.badge ? pendingReviews : undefined}
          ...
          {!collapsed && (
            <div className="text-[10px] text-[var(--text-tertiary)] tracking-[0.08em] uppercase mt-4 mb-1 px-3">
              Developer
            </div>
          )}
          <div>
            ...
            <>
              ...
              {!collapsed && (
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[10px] text-[var(--accent-amber)] font-medium">
                    Guest Mode · {3 - guestSessionsUsed} uses left
                  </span>
                </div>
              )}
            </>
                ...
                <>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-[10px] px-1 bg-[var(--bg-card)] text-[var(--text-secondary)] border border-[var(--border-default)] rounded">
                        {planLabel}
                      </span>
                    </div>
                  </div>
                  <Link to="/dashboard/settings" className="text-[var(--text-tertiary)] hover:text-[var(--text-primary)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col max-h-[350px]">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n: any) => (
                        ...
                        >
                          {!n.is_read && <div className={`absolute left-0 top-0 bottom-0 w-[2px] ${getNotifColor(n.type)}`} />}
                          <div className="pt-0.5 shrink-0">{getNotifIcon(n.type)}</div>
                          <div className="flex-1 min-w-0">
            ...
            <div className="hidden md:block">
              <Dropdown
                align="right"
                trigger={
                  <button className="w-8 h-8 flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded hover:bg-[var(--bg-hover)] transition-colors focus:outline-none" aria-label="Help">
                    <HelpCircle className="w-5 h-5" />
                  </button>
                }
                items={[
                  { label: 'Documentation', icon: <Puzzle className="w-3 h-3" />, onClick: () => window.open('/docs', '_blank') },
                  { label: 'Email Support', icon: <CheckCircle className="w-3 h-3" />, onClick: () => { window.location.href = 'mailto:heyarkvoid@gmail.com'; } },
                ]}
              />
            </div>
            ...
            <div className="hidden md:block">
              <Dropdown
                align="right"
                trigger={
                  <button className="h-[28px] px-3 bg-[var(--accent-amber)] text-black text-[11px] font-bold rounded-[var(--radius-sm)] flex items-center gap-1 hover:bg-[var(--accent-amber-hover)] ml-2 transition-colors focus:outline-none" aria-label="New item">
            ...
            <button
              className="md:hidden ml-1 w-8 h-8 flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded hover:bg-[var(--bg-hover)] transition-colors focus:outline-none"
              onClick={() => setMobileMenuOpen(true)}
              aria-label="Menu"
            >
        ...
        <main className="flex-1 flex flex-col overflow-y-auto overflow-x-hidden relative">
          ...
          <DashboardErrorBoundary>
            ...
            {/* Footer — FIX: was using absolute centering which showed "ARKVOID INC." mid-screen on mobile */}
            <footer className="py-5 px-4 md:px-8 border-t border-[var(--border-subtle)] bg-[var(--bg-card)]/50 backdrop-blur-sm flex flex-col sm:flex-row items-center justify-between gap-3 text-[12px] text-[var(--text-tertiary)] w-full">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-3 gap-y-1">
                <span>© 2026 ARKVOID Inc.</span>
              ...
              <Link to="/status" className="flex items-center gap-1.5 hover:text-white transition-colors group shrink-0">
                <div className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-20" />
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500 group-hover:bg-emerald-400" />
                </div>
                Systems Operational
              </Link>
            </footer>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-[var(--bg-elevated)] border-r border-[var(--border-subtle)] transform transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] md:hidden flex flex-col ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-[52px] flex items-center justify-between px-4 border-b border-[var(--border-subtle)] shrink-0">
          <Logo />
          <button onClick={() => setMobileMenuOpen(false)} className="text-[var(--text-tertiary)] hover:text-[var(--text-primary)] p-1 rounded transition-colors">
        ...
        <div className="flex-1 overflow-y-auto py-4 hide-scrollbar">
          ...
          <div className="mb-6 px-2">
            {workspaceItems.map((item) => (
              <NavItem key={item.path} item={item} collapsed={false} isActive={isActive(item.path)} onMobileClick={() => setMobileMenuOpen(false)} />
            ))}
          </div>
        ...
        <div className="border-t border-[var(--border-subtle)] p-4 flex items-center gap-3 bg-[var(--bg-card)] shrink-0">
          {isGuest ? (
            <>
              <div className="w-[32px] h-[32px] rounded-full bg-[var(--bg-card)] border-2 border-[var(--border-default)] flex items-center justify-center font-bold text-[var(--text-secondary)] text-[14px] shrink-0">G</div>
              <div className="flex-1 min-w-0">
            ...
            <>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-[var(--text-tertiary)]">{planLabel} Plan</div>
              </div>
              <Link to="/dashboard/settings" className="text-[var(--text-tertiary)] hover:text-[var(--text-primary)] p-1 rounded" onClick={() => setMobileMenuOpen(false)}>
      ...
      {liveToast && (
        <div className="fixed top-4 right-4 z-[9999] w-[360px] bg-[var(--bg-elevated)] border border-[var(--border-default)] shadow-2xl rounded-lg p-4 flex gap-3 animate-in slide-in-from-top-2 fade-in duration-300">
          <div className="pt-0.5 shrink-0">{getNotifIcon(liveToast.type)}</div>
          <div className="flex-1 min-w-0">
            ...
            <button
              onClick={() => { window.location.href = liveToast.link; setLiveToast(null); }}
          ...
          <button onClick={() => setLiveToast(null)} className="absolute top-2 right-2 p-1 text-[var(--text-tertiary)] hover:text-[var(--text-primary)] rounded transition-colors">
            ×
          </button>
        </div>
      )}
